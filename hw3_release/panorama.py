...
def harris_corners(img, window_size=3, k=0.04):
    ...
    1)Compute $x$ and $y$ derivatives (dx, dy) of an image
    2) Compute products of derivatives ($I_x^2, I_y^2, I_{xy}$) at each pixel
    3)Compute matrix $M$ at each pixel, where M = \sum_{x,y} w(x,y)
    \begin{bmatrix}
        I_{x}^2 & I_{x}I_{y} \\
        I_{x}I_{y} & I_{y}^2
    \end{bmatrix}
    4)Compute corner response 𝑅=𝐷𝑒𝑡(𝑀)−𝑘(𝑇𝑟𝑎𝑐𝑒(𝑀)2) at each pixel
    ...
    dydy = dy ** 2
    dxdy = dx * dy
    convDx = convolve(dxdx, window)
    convDy = convolve(dydy, window)
    convDxDy = convolve(dxdy, window)
    for h in range(0, H):
        for w in range(0, W):
            f = [convDx[h][w], convDxDy[h][w]]
            s = [convDxDy[h][w], convDy[h][w]]
            m = np.array([f, s])
            trace = np.trace(m)
            response[h][w] = np.linalg.det(m) - k * (trace * trace)
    ### END YOUR CODE
...
def test1():
    ...
    show_image(response)
    show_image(imread('solution_harris.png', as_gray=True))
